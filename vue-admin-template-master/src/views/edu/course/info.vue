...
export default {
  ...
  created() {
    ...
    if (this.$route.params && this.$route.params.id) {
      this.getCourseInfoId(this.$route.params.id)
    }
  },
  methods: {
    getCourseInfoId(id) {
      course.getCourseInfoId(id)
        .then(responese => {
          this.courseInfo = responese.data.course
          this.getTwoList(responese.data.course.subjectParentId)
        })
    },
    // 上传之前
    ...
    getTwoList(subjectParentId) {
      subject.getSubjectList().then((response) => {
        for (let i = 0; i < response.data.list.length; i++) {
          if (response.data.list[i].id == subjectParentId) {
            this.subjectTwoList = response.data.list[i].children
          }
        }
      })
    },
