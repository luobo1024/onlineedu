...
export default {
  ...
  watch: {
    dialogChapterFormVisible(val, oldVal) {
      // eslint-disable-next-line eqeqeq
      if (val == false && oldVal == true) {
        this.chapter = {}
      }
    },
    dialogVideoFormVisible(val, oldVal) {
      // eslint-disable-next-line eqeqeq
      if (val == false && oldVal == true) {
        this.video = {}
      }
  ...
  created() {
    ...
    if (this.$route.params && this.$route.params.id) {
      this.courseId = this.$route.params.id // 获取id
    }
  ...
  methods: {
    ...
    addChapter() {
      this.chapter.courseId = this.courseId// 添加时生成ID
      chapter.addChapterVideo(this.chapter).then((response) => {
    ...
    getChapterVideo() {
      chapter.getChapterVideo(this.courseId).then((response) => {
        this.chapterVideoList = response.data.Videos
      })
    },
    previous() {
